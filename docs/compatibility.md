...
**Status:** Stable **`1.1.0`** (API frozen as of **`1.0.0`**) — **`AbortSignal.timeout`** required when passing **`timeoutMs`** or **`overallTimeoutMs`** (validated at call site; see **`LSM-CORE-65`**).
...
| ------------------ | -------------- | ----------------------------------------------------------------------------------------------------- |
| Node.js 22+        | **Primary CI** | LTS **22, 24** in GitHub Actions — matches `package.json` `engines.node` (`>=22`)                     |
...
GitHub Actions runs **`pnpm verify`** on Node **22 and 24** (includes **`typecheck:examples`** and **`LSM-REL-10*`** / **`LSM-REL-11*`** / **`LSM-REL-12*`**).
